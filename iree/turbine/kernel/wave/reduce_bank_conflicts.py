...
# [row, col] -> [row, (col / vec) ^ ((row // perPhase) % maxPhase) * vec + (col % vec)]
...
def is_shrared_read_write(node: fx.Node) -> bool:
    custom = get_custom(node)
    return (
        isinstance(custom, (Read, Write))
        and subs_idxc(custom.memory_type.address_space) == SHARED_ADDRESS_SPACE
    )
...
def swizzle_shared_memory(trace: CapturedTrace, constraints: list[Constraint]):
    ...
    shared_read_write_nodes = trace.walk(is_shrared_read_write)
    if not shared_read_write_nodes:
        return
    ...
    bankBitWidth = 32
    SIMDWidth = 16
    # TODO: Figure out way to make it 2D or make it MMA specific.
    # TODO: When to swizzle and when to pad.
    # TODO: figure out way to use xor.
    # TODO: Figure out how to match write to shared mem index and read from shared mem.
    # TODO: I think we cannot minimize_global_loads + pad and swizzle at the same time.
    # TODO [Conclusion]: Need to make it MMA specific, for VMMA do swizzle, everything else minimize global load + pad.
    get_thread_shape = lambda index: max(subs_idxc(x.size) for x in index.values())
    get_wg_size = lambda index: subs_idxc(constraint_tile_size.get(index, index))
    for read_write_node in shared_read_write_nodes:
        custom = get_custom(read_write_node)
        vecSize = get_thread_shape(custom.index)
        shared_mem_dims = get_custom(custom.memory).type.symbolic_shape

        # Get per phase/repeat counts.
        bitwidth = custom.type.dtype.bitwidth()
        innerDim = shared_mem_dims[-1]
        elemsPerOneBanksRow = (numBanks * bankBitWidth) // bitwidth
        innerDimLength = get_wg_size(innerDim)
        perPhase = max(1, elemsPerOneBanksRow // innerDimLength)

        outerDims = [dim for dim in shared_mem_dims if dim != innerDim and get_wg_size(dim) != 1]

        # Can only swizzle for cases where there exist only one outer-dim, and is non-one.
        if len(outerDims) != 1:
            continue
        outerDim = outerDims[0]

        # Get max phase
        maxPhase = max(min(SIMDWidth // perPhase, innerDimLength // vecSize), 1)

        # Constructing swizzle expression
        colExpr = read_write_node.index[innerDim].start
        xorByRowExpr = (read_write_node.index[outerDim].start // perPhase) % maxPhase
        vectorOffsetExpr = colExpr % vecSize
        breakpoint()
        read_write_node.index[innerDim].start = sympy.Xor(colExpr, xorByRowExpr) * vecSize + vectorOffsetExpr
