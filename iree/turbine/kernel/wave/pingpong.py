...
"""
# TODO: 1. Add asymetrty before and after loop (done)
# TODO: 2. Heuristic for strategy selection
# TODO: 3. Fix LDS_BARRIER being auto inserted at start of loop. (done)
# TODO: 4. Add a setprio op (done)
...
def reoreder_graph(graph, clusters):
    ...
    assert (
        len(node_list) == total_reordered_node
...
def slice_mma(mma_nodes, lhs_nodes, rhs_nodes, num_slice):
    ...
    # TODO: Move this logic to strategy selection with default strategy if fail.
...
def transform_two_PP_clusters(
    ...
    # TODO: Add dot_slice function that returns sliced global_load and local_load
    ...
    clusters.append(SetPrio(1).add_to_graph(tmp_graph))
    clusters.append(sliced_mma_nodes[0])
    clusters.append(SetPrio(0).add_to_graph(tmp_graph))
    clusters.append(WorkgroupBarrier().add_to_graph(tmp_graph))
    clusters.append(SchedulingBarrier([]).add_to_graph(tmp_graph))
