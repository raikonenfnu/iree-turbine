...
@require_e2e
@pytest.mark.parametrize("shape", get_test_shapes("hyv_attention"))
@pytest.mark.parametrize("enable_scheduling", [SchedulingType.NONE])
@param_bool("dynamic_dims", "dyn", [False])
@pytest.mark.parametrize(
    "mfma_variant",
...
def testHYVAttentionBSHD(
    ...
    if dump_generated_mlir:
        filename = f"hyv_wave_attention_seqlen{shape.query_seq_len}_{mfma_variant[0].name}.mlir"
        with open(filename, "w") as f:
            f.write(asm)
